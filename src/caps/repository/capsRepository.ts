...
export class CapsRepository {
  public static async createCaps(
    ...
    try {
      const sql =
      "INSERT INTO caps (categoria_id, name, price, created_by, updated_by, imagen,talla_id,tipo,descripcion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      const params = [
        categoria_id,
        name,
        price,
  ...
  public static async updateCap(
    ...
    talla:number,
    categoria_id:number
  ): Promise<Cap | string | any> {
    try {
      const sql =
        "UPDATE caps SET name = ?, price = ?, updated_by = ?, imagen = ?,categoria_id=?,tipo=?, descripcion=? WHERE name = ? and talla_id = ?";
      const params = [name, price, updated_by, imagen,categoria_id,tipo,descripcion,name2,talla];
      ...
      return { message: `cap with ID ${name2} updated successfully.` };
    } catch (error: any) {
  ...
  public static async getAllCaps(): Promise<Cap[]> {
    try {
      const sql = "SELECT * FROM caps WHERE deleted = ?";
      const [rows]: any = await query(sql,[false]);
  ...
  public static async getCapsByCategorieId(
    categoria_id: string
  ): Promise<Cap[]> {
    try {
      const sql =
        "SELECT * FROM caps WHERE categoria_id = ? AND deleted = false";
  ...
  public static async deleteCap(name: string) {
    try {
      const sql = "UPDATE caps SET deleted = true WHERE name = ?";
      const params = [name];
      const [result]: any = await query(sql, params);
      ...
      // Retorna un mensaje de éxito junto con el ID actualizado
      return `cap with ID ${name} marked as deleted successfully.`;
    } catch (error: any) {
